...
export const orderRequestSchema = z.object({
  ...
  fiatCurrency: z.enum(["USD", "AED", "IQD", "SAR", "QAR", "KWD", "BHD", "OMR", "EUR", "GBP"]),
  ...
  const address = value.walletAddress;
  if (/seed|mnemonic|private\s*key/i.test(address) || /^0x[a-fA-F0-9]{64}$/.test(address)) {
    context.addIssue({ code: "custom", path: ["walletAddress"], message: "Secret material is not allowed" });
    return;
  }
  const valid =
    value.network === "TRC20"
